...
func (gs *gatewayService) udpLoop() {
	...
	var temp [1500]byte
	var iprot protocol.IProtocol = protocol.NewBinaryProtocol()
	for {
		...
		// 设定生存周期
		{
			...
			iprot.Release()
			iprot.Write(temp[:dLen])
			
			if _ , _ , err :=	messages.UnMarshalTStruct(context.Background(),iprot,request); err != nil {
		...
		// 查找目标路由
		r := gs.gwy.FindRouter("OnReportNat")
		if r == nil {
		...
		// 推送到目标服务
		if _, err := r.Proxy.RequestMessage(postRequest,"OnReportNat", 2000); err != nil {
