...
func LogProcessor(profiles []*TagsProfile) error {
	...
	for _, p := range profiles {
		if p.Key != "" {
			vlog.Infof("VELCRO: profiler - %s %.2f%%", p.Key, p.Percent*100)
		} else {
			vlog.Infof("VELCRO: profiler - type=default %.2f%%", p.Percent*100)
		}
		vlog.Info("---------------------------------")
	}
	return nil
...
func (p *profiler) Run(ctx context.Context) (err error) {
	...
	for {
		...
		if err != nil {
			vlog.Errorf("VELCRO: profiler analyse failed: %v", err)
			continue
		}
		err = p.processor(profiles)
		if err != nil {
			vlog.Errorf("VELCRO: profiler controller process failed: %v", err)
			continue
		}
