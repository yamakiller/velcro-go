...
func (c *longConn) IsActive() bool {
	if conn, ok := c.Conn.(client.IsActive); ok {
...
func (p *peer) Get(d client.Dialer, timeout time.Duration, reporter Reporter, addr string) (net.Conn, error) {
	var c net.Conn
	c, reused, decNum := p.pool.Get()
	p.globalIdle.DecN(int64(decNum))
	if reused {
		reporter.ReuseSucceed(p.serviceName, p.addr)
		return c, nil
	}
	// dial a new connection
	c, err := d.DialTimeout(p.addr.Network(), p.addr.String(), timeout)
	if err != nil {
		reporter.ConnFailed(p.serviceName, p.addr)
		return nil, err
	}
	reporter.ConnSucceed(p.serviceName, p.addr)
	return &longConn{
		Conn:    c,
		address: addr,
	}, nil
...
func NewLongPool(serviceName string, idlConfig IdleConfig) *LongPool {
	limit := syncx.NewMaxCounter(idlConfig.MaxIdleGlobal)
	lp := &LongPool{
...
func (lp *LongPool) Get(ctx context.Context, network, address string, opt client.ConnOption) (net.Conn, error) {
