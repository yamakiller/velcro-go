...
import (
	...
	"github.com/yamakiller/velcro-go/rpc/pkg/retry"
...
// NewDefaultCodec 创建一个默认编码器,单数据包大小无限制
...
// NewDefaultCodecWithSizeLimit 创建一个默认编码器, 设置单数据包大小限制
...
func (c *defaultCodec) EncodePayload(ctx context.Context, message remote.Message, out remote.ByteBuffer) error {
	defer func() {
		if rio := message.RPCInfo(); rio != nil {
			if msg := rpcinfo.AsMutableRPCStats(rio.Stats()); msg != nil {
				msg.SetSendSize(uint64(out.MallocLen()))
			}
		}
	}()
	...
	var (
		err               error
		framedLengthField []byte
	)

	headerLength := out.MallocLen()
	tp := message.ProtocolInfo().TransProto

	// 1. 如果需要分配帧字段
	if tp&transport.Framed == transport.Framed {
		if framedLengthField, err = out.Malloc(Size32); err != nil {
			return err
		}
		headerLength += Size32
	}
	...
	if tp&transport.Framed == transport.Framed {
		payloadLength = out.MallocLen() - headerLength
		binary.BigEndian.PutUint32(framedLengthField, uint32(payloadLength))
	...
	payloadLength = out.MallocLen() - Size32
	err = verifyPayloadSize(payloadLength, c.maxSize)
...
func (c *defaultCodec) EncodeMetaAndPayload(ctx context.Context, message remote.Message, out remote.ByteBuffer, me remote.MetaEncoder) error {
	var (
		err             error
		dataLengthField []byte
	)

	// 1. 编码头
	if dataLengthField, err = velcroHeaderCodec.encode(ctx, message, out); err != nil {
		return err
	}
	...
	// 3.填充数据块总长度
	payloadLength := out.MallocLen() - Size32 - VelcroFixedHeaderSize
	binary.BigEndian.PutUint32(dataLengthField, uint32(payloadLength))
...
func (c *defaultCodec) DecodeMeta(ctx context.Context, message remote.Message, in remote.ByteBuffer) (err error) {
	...
	return verifyPayload(flagBuf, message, in, c.maxSize)
}
...
// verifyPayload 校验数据体
func verifyPayload(flagBuf []byte, message remote.Message, in remote.ByteBuffer, maxPayloadSize int) error {
	var (
